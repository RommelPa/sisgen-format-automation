...
from sisgen_automation.traene.template import TRAENE_HEADERS, parse_period
...
@dataclass(frozen=True)
class TraeneTemplateValidationResult:
    template_path: Path
    period: str
    records: list[TraeneTemplateRecord]
    issues: list[SimpleTemplateIssue]
...
def validate_traene_template(
    *,
    ...
    if not template_path.exists():
        raise ValueError(f"No existe la plantilla TRAENE: {template_path}")
    ...
    expected_by_code = {item.ccodgen: item for item in catalog.power_transfers}

    workbook = load_workbook(template_path, data_only=True)
    sheet = workbook.active
    rows = list(sheet.iter_rows(values_only=True))
    header_row = _find_header_row(rows, TRAENE_HEADERS)
    ...
    for row_index, row in enumerate(rows[header_row:], start=header_row + 1):
        values = list(row[: len(TRAENE_HEADERS)])

        if all(value is None or str(value).strip() == "" for value in values):
            continue

        raw = dict(zip(TRAENE_HEADERS, values, strict=True))
        ...
        value = _require_decimal(raw=raw, field="NTRAPOT", row_index=row_index, issues=issues)
    ...
    return TraeneTemplateValidationResult(template_path=template_path, period=period, records=records, issues=issues)
