...
from sisgen_automation.traene.template import parse_period
from sisgen_automation.traene.template_validation import (
    TraeneTemplateRecord,
    validate_traene_template,
)
...
def default_traene_output_path(period: str) -> Path:
    return Path("reports") / "dbf" / period / "TRAENE.DBF"


def _record_to_mapping(record: TraeneTemplateRecord) -> dict[str, object]:
    return {
        "CANOREG": record.canoreg,
        "CMESREG": record.cmesreg,
        "CCODEMP": record.ccodeemp,
        "CCODGEN": record.ccodgen,
        "CNOMGEN": record.cnomgen,
        "NTRAPOT": record.ntrapot,
    }


def export_traene_dbf(
    *,
    source_dbf_path: Path,
    template_path: Path,
    period: str,
    ...
    validation_result = validate_traene_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla TRAENE tiene errores. Corrige la plantilla antes de exportar el DBF.")
    ...
    if output_path is None:
        output_path = default_traene_output_path(period)
