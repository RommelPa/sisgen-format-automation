...
from sisgen_automation.cacehi.template import parse_period
...
def default_cacehi_output_path(period: str) -> Path:
    return Path("reports") / "dbf" / period / "CACEHI.DBF"
...
def _record_to_mapping(record: CacehiTemplateRecord) -> dict[str, object]:
    return {
        ...
        "CNOMCUE": record.cnomcue,
        "NCAMEDI": record.ncamedi,
        "NALVOUT": record.nalvout,
    }
...
def export_cacehi_dbf(
    *,
    ...
    validation_result = validate_cacehi_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla CACEHI tiene errores. Corrige la plantilla antes de exportar el DBF.")
    ...
    if output_path is None:
        output_path = default_cacehi_output_path(period)
