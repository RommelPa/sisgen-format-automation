...
def default_cacehi_template_path(period: str) -> Path:
    year_text, month_text = period.split("-", maxsplit=1)
    return Path("reports") / "templates" / f"CACEHI_{year_text}_{month_text}_template.xlsx"


def create_cacehi_template(
    *,
    period: str,
    catalog_path: Path,
    ...
    if output_path is None:
        output_path = default_cacehi_template_path(period)
    ...
    for col_index, header in enumerate(CACEHI_HEADERS, start=1):
    ...
    for row_index, unit in enumerate(catalog.hydro, start=2):
        values = {
            "CANOREG": year_short,
            "CMESREG": month,
            "CCODGEN": catalog.company.ccodgen,
            "CCODCEN": unit.ccodcen,
            "CNOMCEN": unit.cnomcen,
            "CNOMCUE": unit.cnomcue,
            "NCAMEDI": None,
            "NALVOUT": None,
        }
    ...
    sheet.auto_filter.ref = f"A1:H{1 + len(catalog.hydro)}"
    ...
    return CacehiTemplateResult(
        period=period,
        output_path=output_path,
        rows=len(catalog.hydro),
    )