...
from sisgen_automation.comcen.template import parse_period
from sisgen_automation.comcen.template_validation import (
    ComcenTemplateRecord,
    validate_comcen_template,
)
...
def default_comcen_output_path(period: str) -> Path:
    return Path("data") / "output" / period / "COMCEN.DBF"
...
def _format_character(value: object, field: DbfField) -> bytes:
    ...
    return text.ljust(field.length).encode("latin-1", errors="replace")
...
def _record_to_mapping(record: ComcenTemplateRecord) -> dict[str, object]:
    return {
        "CANOREG": record.canoreg,
        "CMESREG": record.cmesreg,
        "CCODEMP": record.ccodemp,
        "CCODCEN": record.ccodcen,
        "CTIPGRU": record.ctipgru,
        "CNOMNUM": record.cnomnum,
        "CCODCOM": record.ccodcom,
        "CDESCOM": record.cdescom,
        "NTOTCOM": record.ntotcom,
    }
...
def export_comcen_dbf(
    *,
    ...
    validation_result = validate_comcen_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla COMCEN tiene errores. Corrige la plantilla antes de exportar el DBF.")
    ...
    if output_path is None:
        output_path = default_comcen_output_path(period)
