...
from sisgen_automation.comene.template import parse_period
from sisgen_automation.comene.template_validation import (
    EnergyTemplateRecord,
    validate_comene_template,
)
...
def _period_exists(source_dbf_path: Path, period: str, parse_period_func) -> bool:
...
def _write_appended_dbf(
    *,
    ...
    output_path: Path,
    record_mappings: list[dict[str, object]],
):
    ...
    return profile.record_count, len(serialized_records), final_record_count




@dataclass(frozen=True)
class ComeneExportResult:
    period: str
    output_path: Path
...
def default_comene_output_path(period: str) -> Path:
    return Path("reports") / "dbf" / period / "COMENE.DBF"



def _record_to_mapping(record: EnergyTemplateRecord) -> dict[str, object]:
    return {
        "CANOREG": record.canoreg,
        "CMESREG": record.cmesreg,
        "CCOSISI": record.ccosisi,
        "CCODEMP": record.ccodeemp,
        "CCODGEN": record.ccodgen,
        "CDESGEN": record.cdesgen,
        "NENHOPU": record.nenhopu,
        "NENFUHO": record.nenfuho,
        "NENETOT": record.nenetot,
        "NVAHOPU": record.nvahopu,
        "NVAFUHO": record.nvafuho,
        "NVALTOT": record.nvaltot,
    }


def export_comene_dbf(
    *,
    source_dbf_path: Path,
    template_path: Path,
    period: str,
    ...
    validation_result = validate_comene_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla COMENE tiene errores. Corrige la plantilla antes de exportar el DBF.")

    if not allow_existing_period and _period_exists(source_dbf_path, period, parse_period):
        raise ValueError(
    ...
    if output_path is None:
        output_path = default_comene_output_path(period)

    original_count, appended_count, final_count = _write_appended_dbf(
        source_dbf_path=source_dbf_path,
        output_path=output_path,
        record_mappings=[_record_to_mapping(record) for record in validation_result.records],
    )

    return ComeneExportResult(
        period=period,
        output_path=output_path,
