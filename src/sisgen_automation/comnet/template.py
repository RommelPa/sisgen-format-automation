...
EDITABLE_HEADERS = {"NCOMNET"}
NUMERIC_HEADERS = {"NCOMNET"}


@dataclass(frozen=True)
class ComnetTemplateResult:
    period: str
    output_path: Path
    rows: int
...
def default_comnet_template_path(period: str) -> Path:
    year_text, month_text = period.split("-", maxsplit=1)
    return Path("reports") / "templates" / f"COMNET_{year_text}_{month_text}_template.xlsx"


def create_comnet_template(
    *,
    period: str,
    catalog_path: Path,
    output_path: Path | None = None,
) -> ComnetTemplateResult:
    year_short, month = parse_period(period)
    catalog = load_g7_catalog(catalog_path)

    if output_path is None:
        output_path = default_comnet_template_path(period)
    ...
    for col_index, header in enumerate(COMNET_HEADERS, start=1):
    ...
    for row_index, party in enumerate(catalog.net_commitments, start=2):
        values = {
            "CANOREG": year_short,
            "CMESREG": month,
            "CCODEMP": catalog.company.ccodeemp,
            "CCODDIS": party.ccoddis,
            "CNOMDIS": party.cnomdis,
            "NCOMNET": None,
        }
    ...
    sheet.auto_filter.ref = f"A1:F{1 + len(catalog.net_commitments)}"
    ...
    return ComnetTemplateResult(
        period=period,
        output_path=output_path,
        rows=len(catalog.net_commitments),
    )