...
from sisgen_automation.comnet.template import parse_period
from sisgen_automation.comnet.template_validation import (
    ComnetTemplateRecord,
    validate_comnet_template,
)
...
def default_comnet_output_path(period: str) -> Path:
    return Path("reports") / "dbf" / period / "COMNET.DBF"


def _record_to_mapping(record: ComnetTemplateRecord) -> dict[str, object]:
    return {
        "CANOREG": record.canoreg,
        "CMESREG": record.cmesreg,
        "CCODEMP": record.ccodeemp,
        "CCODDIS": record.ccoddis,
        "CNOMDIS": record.cnomdis,
        "NCOMNET": record.ncomnet,
    }


def export_comnet_dbf(
    *,
    source_dbf_path: Path,
    template_path: Path,
    period: str,
    ...
    validation_result = validate_comnet_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla COMNET tiene errores. Corrige la plantilla antes de exportar el DBF.")
    ...
    if output_path is None:
        output_path = default_comnet_output_path(period)
