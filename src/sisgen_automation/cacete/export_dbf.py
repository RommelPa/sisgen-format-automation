...
from sisgen_automation.cacete.template import parse_period
...
def default_cacete_output_path(period: str) -> Path:
    return Path("reports") / "dbf" / period / "CACETE.DBF"
...
def _record_to_mapping(record: CaceteTemplateRecord) -> dict[str, object]:
    return {
        "CANOREG": record.canoreg,
        "CMESREG": record.cmesreg,
        "CCODGEN": record.ccodgen,
        "CCODCEN": record.ccodcen,
        "CNOMCEN": record.cnomcen,
        "CTIPCOM": record.ctipcom,
        "CDESCOM": record.cdescom,
        "NVOLALM": record.nvolalm,
        "NADQMES": record.nadqmes,
    }
...
def export_cacete_dbf(
    *,
    ...
    validation_result = validate_cacete_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla CACETE tiene errores. Corrige la plantilla antes de exportar el DBF.")
    ...
    if output_path is None:
        output_path = default_cacete_output_path(period)
