...
EDITABLE_HEADERS = {"NVALTRA"}
NUMERIC_HEADERS = {"NVALTRA"}


@dataclass(frozen=True)
class ValeneTemplateResult:
    period: str
    output_path: Path
    rows: int
...
def default_valene_template_path(period: str) -> Path:
    year_text, month_text = period.split("-", maxsplit=1)
    return Path("reports") / "templates" / f"VALENE_{year_text}_{month_text}_template.xlsx"


def create_valene_template(
    *,
    period: str,
    catalog_path: Path | None = None,
    catalog_db_path: Path | None = None,
    output_path: Path | None = None,
) -> ValeneTemplateResult:
    ...
    if output_path is None:
        output_path = default_valene_template_path(period)
    ...
    for col_index, header in enumerate(VALENE_HEADERS, start=1):
    ...
    for row_index, party in enumerate(catalog.transfer_valuations, start=2):
        values = {
            "CANOREG": year_short,
            "CMESREG": month,
    ...
    sheet.auto_filter.ref = f"A1:F{1 + len(catalog.transfer_valuations)}"
    ...
    return ValeneTemplateResult(
        period=period,
        output_path=output_path,
        rows=len(catalog.transfer_valuations),
    )