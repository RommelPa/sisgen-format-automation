...
from sisgen_automation.valene.template import parse_period
from sisgen_automation.valene.template_validation import (
    ValeneTemplateRecord,
    validate_valene_template,
)
...
def default_valene_output_path(period: str) -> Path:
    return Path("reports") / "dbf" / period / "VALENE.DBF"


def _record_to_mapping(record: ValeneTemplateRecord) -> dict[str, object]:
    return {
        ...
        "NVALTRA": record.nvaltra,
    }


def export_valene_dbf(
    *,
    source_dbf_path: Path,
    template_path: Path,
    period: str,
    ...
    validation_result = validate_valene_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla VALENE tiene errores. Corrige la plantilla antes de exportar el DBF.")
    ...
    if output_path is None:
        output_path = default_valene_output_path(period)
