...
from sisgen_automation.g2.template import DBF_ENCODING, parse_period
...
def default_vepoen_output_path(period: str) -> Path:
    return Path("reports") / "dbf" / period / "VEPOEN.DBF"
...
def _record_to_mapping(record: VepoenTemplateRecord) -> dict[str, object]:
    return {
        "CANOREG": record.canoreg,
        "CMESREG": record.cmesreg,
        "CCODEMP": record.ccodemp,
        "CCODDIS": record.ccoddis,
        "CSIELDI": record.csieldi,
        "CCOSUES": record.ccosues,
        "CNOMBAR": record.cnombar,
        "CCLRELI": record.cclreli,
        "NTENBAR": record.ntenbar,
        "NPRPOBA": record.nprpoba,
        "NPRENHO": record.nprenho,
        "NPRENFU": record.nprenfu,
        "NENVEHO": record.nenveho,
        "NENVEFU": record.nenvefu,
        "NENVETO": record.nenveto,
        "NPOCOHO": record.npocoho,
        "NPOCOFU": record.npocofu,
        "NMADEHO": record.nmadeho,
        "NMADEFU": record.nmadefu,
        "NFACPOT": record.nfacpot,
        "NFACENE": record.nfacene,
    }
...
def export_vepoen_dbf(
    *,
    ...
    validation_result = validate_vepoen_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla VEPOEN tiene errores. Corrige la plantilla antes de exportar el DBF.")
    ...
    if output_path is None:
        output_path = default_vepoen_output_path(period)
