...
EDITABLE_COLUMNS = {"H", "K", "L", "N", "O", "P", "Q"}
...
def _write_instructions_sheet(workbook: Workbook, period: str) -> None:
    ...
    rows = [
        ...
        ("Columnas editables", "CESTGRU, NHORPUN, NFUHOPU, CHRSMAN, CHRSOPE, CHRSSAL, NCONLUB."),
        ("Columna calculada", "NTOPRBR se calcula automáticamente como NHORPUN + NFUHOPU."),
        ("NCONLUB", "Consumo de lubricantes."),
        ("Advertencia", "No modificar códigos, empresa, tipo de grupo ni nombre de grupo."),
...
def _write_header(sheet) -> None:
    ...
    sheet.auto_filter.ref = f"A1:{get_column_letter(len(CENTER_HEADERS))}1"
...
def _write_unit_row(
    ...
    for column_index, value in enumerate(values, start=1):
        ...
        if column_letter in {"I", "J", "K", "L", "M", "N", "O", "P", "Q"}:
...
def _apply_sheet_format(sheet, last_row: int) -> None:
    ...
    sheet["Q1"].comment = Comment("Consumo de lubricantes.", "SISGEN")
