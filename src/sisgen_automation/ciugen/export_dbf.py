...
def default_ciugen_output_path(period: str) -> Path:
    return Path("reports") / "dbf" / period / "CIUGEN.DBF"
...
def _serialize_record(
    *,
    ...
    if len(serialized) != record_length:
        raise ValueError(
            f"Registro serializado con longitud invalida. "
...
def _record_to_mapping(record: CiugenTemplateRecord) -> dict[str, object]:
    ...
    *,
    ...
    if len(source_bytes) < expected_data_end:
        raise ValueError(
            ...
            f"Tamanio esperado minimo: {expected_data_end}. Tamanio real: {len(source_bytes)}."
        )
...
def export_ciugen_dbf(
    *,
    ...
    validation_result = validate_ciugen_template(
    ...
    if validation_result.has_errors:
        raise ValueError("La plantilla CIUGEN tiene errores. Corrige la plantilla antes de exportar el DBF.")

    if not allow_existing_period and _period_exists(source_dbf_path, period):
        raise ValueError(
            f"El periodo {period} ya existe en el DBF historico. "
            "No se exporto para evitar duplicar informacion."
        )

    if output_path is None:
        output_path = default_ciugen_output_path(period)
